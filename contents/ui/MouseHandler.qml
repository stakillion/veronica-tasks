    ...
    WheelHandler {
        ...
        onWheel: event => {
            ...
            if (Plasmoid.configuration.wheelEnabled === 3) {
                const pa = tasks?.pulseAudio?.item;
                if (!pa) {
                    return;
                }
                const loudest = anchor?.audioStreams?.reduce((loudest, stream) => Math.max(loudest, stream.volume), 0)
                const step = (pa.normalVolume - pa.minimalVolume) * pa.globalConfig.volumeStep / 100;
                anchor?.audioStreams?.forEach((stream) => {
                    ...
                    stream.model.Volume = Math.max(pa.minimalVolume, Math.min(volume, pa.normalVolume));
