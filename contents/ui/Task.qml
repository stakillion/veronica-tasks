...
PlasmaCore.ToolTipArea {
    ...
    // Band 1: Card 1 (5px peeking band for 2 windows, 4px for 3+ windows)
    Item {
        id: c1Container
                ...
                ShapePath {
                    ...
                    fillGradient: RadialGradient {
                        centerX: (taskHoverHandler.hovered && taskHoverHandler.point?.position?.x !== undefined
                            ? taskHoverHandler.point.position.x - (frame.x + (frame.windowCount > 2 ? 4 : 5))
                            : frame.glowCenterX - (frame.windowCount > 2 ? 4 : 5))
    ...
    Rectangle {
        ...
        width: isMultiWindow ? (windowCount > 2 ? parent.width - 12 : parent.width - 9) : (parent.width - 4)
        anchors.top: parent.top
        anchors.topMargin: 1
        anchors.bottom: parent.bottom
        anchors.bottomMargin: 1
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.horizontalCenterOffset: isMultiWindow ? (windowCount > 2 ? -4 : -2.5) : 0
        radius: 4
        clip: true
